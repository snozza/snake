(ns snake.server.routes
  (:require
    [snake.server.model :as model]
...
(defmethod event :snake/dir [{:as ev-msg :keys [event uid ?data]}]
  (let [[dx dy] ?data]
    (model/dir uid dx dy)))

(defmethod event :snake/username [{:as ev-msg :keys [event uid ?data]}]
  (model/username uid ?data))

(defmethod event :snake/respawn [{:as ev-msg :keys [event uid ?data]}]
...
(defn broadcast []
  (doseq [uid (:any @(:connected-uids channel-socket))]
    ((:send-fn channel-socket) uid [:snake/world @model/world])))
